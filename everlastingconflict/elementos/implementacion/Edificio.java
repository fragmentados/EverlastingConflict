 ...
 public class Edificio extends ElementoAtacante {
         ...
         if (nombre.equals("Distorsionador temporal")) {
             ...
             for (Jugador enemy : enemies) {
                 for (Unidad u : enemy.unidades) {
                     if (this.alcance(200, u)) {
                         u.statusEffectCollection.anadir_estado(new StatusEffect(StatusEffectName.RALENTIZACION, 1f,
                                 40f));
                     }
                 }
             }
             ...
             for (int i = 0; i < partida.recursos.size(); i++) {
                 ...
                 if (r.nombre.equals("Hierro")) {
                     Rectangle2D r2 = new Rectangle2D.Float(r.x - r.anchura / 2, r.y - r.altura / 2, r.anchura,
                             r.altura);
                     if (r2.intersects(re)) {
                         Jugador aliado = partida.getPlayerFromElement(WindowMain.combatWindow.constructor);
     ...
     public void enable(Unidad enabler, Partida p) {
         Jugador aliado = p.getPlayerFromElement(this);
         if (nombre.equals("Torreta defensiva")) {
             turretTransformation(enabler);
         } else {
             this.activo = true;
             iniciarbotones(p);
